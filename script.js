...
const loanPercent = document.getElementById('loan-percent');
...
const loanBet = document.querySelector('.loan-calculator__result-text-3');
let loanSumValue = 0;
let loanTermValue = 1;
let loanPercentValue = 0;
loanSum.onchange = function () {
   loanSumValue = parseInt(loanSum.value);
   loanPayments();
   loanOverpayments();
}
loanTerm.onchange = function () {
   loanTermValue = parseInt(loanTerm.value);
   loanPayments();
   loanOverpayments();
}
loanPercent.onchange = function () {
   loanPercentValue = parseInt(loanPercent.value).toFixed(0);
   loanPayments();
   loanOverpayments();
   loanBet.textContent = loanPercentValue;
}
